            ...
            <div class="tab-content">
                <div class="left">
                    <div v-for="(value,index) in v.orderProduct" :key="index" class="bar">
                        <div class="name">
                            <img :src="value.specImg?value.specImg:'../../../../assets/images/avatar.jpg'" alt="">
                            <img v-if="value.specImg" :src="value.specImg" alt="">
                ...
                <div class="right">
                    <div v-for="(value,index) in v.orderProduct" :key="index" class="bar">
                        ...
                        <div class="operate">
                            <el-button type="primary">{{value.status}}</el-button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                //         starColor: '#e7e7e7',
                //         isShowPop: false,
                //         orderNum: "201806200508",
                //         createTime: "2018-12-24 12:12:12",
                //         starLevel: '',
                //         markTip: '',
                //         totalPrice: '9999',
                //         postAge: '9',
                //         status: '1',
                //         orderProduct: [{
                //             name: 'Apple/苹果iPhone 8 Plus 64G全网通4G手机',
                //             spec: 'iphone金色128G',
                //             price: '1288',
                //             num: '2',
                //             people: '张三',
                ...
                //     },
                // ],
                markArr: [{label: "red", value: '1'}, {label: "skyblue", value: '2'}, {
                    label: "green",
                    value: '3'
                }, {label: "blue", value: '4'}, {label: "purple", value: '5'}, {label: "black", value: '6'}],
                tableData:[],
                tableLoading:false,
                page: {
                    currentPage: 1,
        ...
        mounted() {
            ...
            if (n == "allOrder") {//所有订单
                this.url = "";
                this.status=''
            } else if (n == "toBePaid") {//待支付订单
                this.url = "4";
                this.status='待支付';
            } else if (n == "toBeSend") {//待发货订单
                this.url = "5";
                this.status='待发货';
            } else if (n == "auditProduct") {//待支付订单
                this.url = "";
                this.status='待支付';
            } else if (n == "toBeStay") {//待自提订单
                this.url = api.queryPickUpByCustomerOrderPageList;
                this.priUrl = pApi.queryPickUpByCustomerOrderPageList;
                this.status='待自提';
            }else if (n == "toBeConfirm") {//待确认订单
                this.url = "3";
                this.status='待确认';
            }else if (n == "refund") {//退款中订单
                this.url = "3";
                this.status='退款中';
            }else if (n == "finished") {//已完成订单
                this.url = "3";
                this.status='已完成';
            }else if (n == "closed") {//已关闭订单
                this.url = "3";
                this.status='已关闭';
            }else if (n == "freeze") {//已冻结订单
                this.url = "3";
                this.status='已冻结';
            }
            this.pControl();
