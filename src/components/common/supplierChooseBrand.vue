...
<script>
    ...
    export default {
        ...
        props: ['addOrUp', 'brandids', 'detailBrand'],
        data() {
            return {
                brandName: '',
                allBrandList: [],
        ...
        methods: {
            ...
            addBrand() {
                ...
                for (let i in that.brandIds) {
                    ...
                    for (let j in that.allBrandList) {
                        if (that.allBrandList[j].id == that.brandIds[i]) {
            ...
            delItem(item, index) {
                ...
                if (that.chooseList[index].checked) {
                    if (that.delIds.indexOf(item.id) == -1) {
                        that.delIds.push(item.id);
                    }
                } else {
                    for (let i in that.delIds) {
                        if (item.id == that.delIds[i]) {
            ...
            delBrand() {
                ...
                for (let i in that.delIds) {
                    for (let j in that.chooseList) {
                        if (that.delIds[i] == that.chooseList[j].id && that.chooseList[j].checked) {
                            let temp = {
                                id: that.chooseList[j].id,
                                name: that.chooseList[j].name,
                                checked: false
                            };
